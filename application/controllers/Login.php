...
class Login extends CI_Controller {
	...
	function index() {
		...
		if ($this->db->table_exists('crud_users')){
			...
			echo "SYSTEM REQUIREMENT: SQL TABLE crud_users DOESN'T EXISTS BUT... Hey, we can do it for you, if the database connection is configured correctly ;)<p><a href=\"".base_url()."index.php/login/createDBTable\"><button>Create the required table in your MySQL database</button></a></p>Oh, a user 'admin' (password 'admin') will be create too, so you can log in directly! Just click on the button!";
	...
	function createDBTable(){
		if ($this->db->table_exists('crud_users')){
			...
			echo '<p><a href="'.base_url().'index.php/login"><button>Go to the Login Page</button></a></p>'; 
		}
	}
